...
test -f /etc/resolv.conf && echo "/etc/resolv.conf is regular file" || echo "/etc/resolv.conf does not regular file"
...
[ -a /tmp ] && echo "/tmp is accessed" || echo "/tmp is not accessed"
